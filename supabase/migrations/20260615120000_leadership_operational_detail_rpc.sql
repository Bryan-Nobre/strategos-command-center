-- BLOCO 3.2: RPC detalhe operacional da rede por liderança (paginação + segmentos)

CREATE INDEX IF NOT EXISTS idx_supporters_tenant_neighborhood
  ON public.supporters(tenant_id, neighborhood)
...
BEGIN
  ...
  RETURN (
    ...
    territory_rows AS (
      SELECT
        COALESCE(l.neighborhood, 'Sem bairro') AS neighborhood,
        COUNT(*)::int AS cnt
      FROM linked l
      GROUP BY COALESCE(l.neighborhood, 'Sem bairro')
...
COMMENT ON FUNCTION public.get_leadership_operational_detail IS
  'Detalhe operacional da rede de uma liderança: summary, apoiadores paginados, território top-5, crescimento.';

GRANT EXECUTE ON FUNCTION public.get_leadership_operational_detail(uuid, text, text, int, int) TO authenticated;

REVOKE ALL ON FUNCTION public.get_leadership_operational_detail(uuid, text, text, int, int) FROM PUBLIC, anon;
