-- P1.3a-hardening: telefone persistido normalizado (todos os paths via trigger).

CREATE OR REPLACE FUNCTION public.supporters_phone_before()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  IF NEW.phone IS NOT NULL THEN
    NEW.phone := public.normalize_supporter_phone(NEW.phone);
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS supporters_phone_normalize ON public.supporters;
CREATE TRIGGER supporters_phone_normalize
  BEFORE INSERT OR UPDATE OF phone
  ON public.supporters
  FOR EACH ROW
  EXECUTE FUNCTION public.supporters_phone_before();

-- Backfill histórico (idempotente)
UPDATE public.supporters s
SET phone = public.normalize_supporter_phone(s.phone)
WHERE s.phone IS NOT NULL
  AND s.phone IS DISTINCT FROM public.normalize_supporter_phone(s.phone);

-- Landing RPC: persistência explícita alinhada ao dedup
