-- BLOCO 2: filtro de relatórios via supporter_leadership_links

-- Corrige agregaÃ§Ãµes JSON e validaÃ§Ã£o de UUID opcional na RPC de relatÃ³rios.
...
BEGIN
  ...
  IF p_from IS NULL OR p_to IS NULL OR p_from > p_to THEN
    RAISE EXCEPTION 'PerÃ­odo invÃ¡lido: verifique as datas de inÃ­cio e fim';
  END IF;
  ...
  SELECT COALESCE(
    json_agg(json_build_object('label', to_char(bucket, 'YYYY-MM'), 'apoiadores', cnt) ORDER BY bucket),
    '[]'::json
  ) INTO v_growth
  FROM (
