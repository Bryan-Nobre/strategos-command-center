-- Hotfix: triggers de atividade com casts enum explícitos (landing + liderança primária).
-- Sem político: INSERT em supporters OK. Com político: INSERT em supporter_leadership_links
-- disparava trg_sll_log_activity_event com tipos text → erro 42883.
...
BEGIN
  ...
  PERFORM public.log_supporter_activity_event(
    NEW.tenant_id,
    NEW.supporter_id,
    'demand_created'::public.supporter_activity_event_type,
    CASE NEW.source
      WHEN 'landing' THEN 'landing'::public.supporter_activity_event_source
...
BEGIN
  IF COALESCE(current_setting('app.recomputing_activity', true), '') = 'on' THEN
    RETURN NEW;
  END IF;
  ...
  IF TG_OP = 'UPDATE' THEN
    PERFORM public.log_supporter_activity_event(
      NEW.tenant_id,
      NEW.id,
      'supporter_updated'::public.supporter_activity_event_type,
      CASE NEW.source
        WHEN 'landing' THEN 'landing'::public.supporter_activity_event_source
        WHEN 'import' THEN 'import'::public.supporter_activity_event_source
        ELSE 'crm'::public.supporter_activity_event_source
...
-- Evita conflito idx_sll_one_primary_per_supporter ao vincular liderança na landing.
