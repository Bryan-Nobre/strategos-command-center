-- Dashboard operacional: RPC única (substitui get_tenant_dashboard_metrics).
-- Agregações no servidor; timezone America/Sao_Paulo para janelas diárias/semanais.

DROP FUNCTION IF EXISTS public.get_tenant_dashboard_metrics(uuid);
...
BEGIN
  ...
  enriched AS (
    SELECT
      ...
      CASE
        ...
        ELSE round(
          ((s.score - b.min_score)::numeric / greatest(b.max_score - b.min_score, 1)) * 100
        )::int
  ...
  RETURN json_build_object(
    ...
    'weekly_goals', v_weekly_goals,
    'unassigned_open_demands', v_unassigned
  );
END;
$$;

REVOKE ALL ON FUNCTION public.get_tenant_operational_dashboard(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_tenant_operational_dashboard(uuid) TO authenticated;
