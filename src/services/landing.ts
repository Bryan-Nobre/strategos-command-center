...
export async function registerFromLanding(
  ...
  const { data, error } = await supabase.rpc("register_supporter_from_landing", {
    p_public_code: publicCode.trim().toLowerCase(),
    p_name: payload.name,
    p_phone: normalizeSupporterPhone(payload.phone) ?? undefined,
    p_email: payload.email,
    p_birth_date: payload.birthDate,
    p_street: payload.street ?? undefined,
    p_address_number: payload.addressNumber ?? undefined,
    p_address_complement: payload.addressComplement ?? undefined,
    p_state_uf: payload.stateUf ?? undefined,
    ...
    p_primary_leadership_id: payload.primaryLeadershipId ?? undefined,
  });
  if (error) throw error;
