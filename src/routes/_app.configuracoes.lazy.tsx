import { createLazyFileRoute, getRouteApi, useRouteContext } from "@tanstack/react-router";
...
const configuracoesRoute = getRouteApi("/_app/configuracoes");

export const Route = createLazyFileRoute("/_app/configuracoes")({
...
function ConfigPage() {
  ...
  const urlSearch = configuracoesRoute.useSearch();
  const navigate = configuracoesRoute.useNavigate();
  ...
  const savedLandingSnapshot = useMemo((): LandingSettingsSnapshot | null => {
    if (!landing) return null;
    return snapshotFromLandingRow(landing);
  }, [landing]);
  ...
  function setTab(tab: ConfigTab) {
    ...
    if (
      ...
      activeTab === "landing" &&
      next !== "landing" &&
      !window.confirm("Você tem alterações não salvas na landing. Deseja sair sem salvar?")
    ) {
      return;
    }
    void navigate({
          ...
          {tabPerms.canEditLanding && (
            <TabsContent value="landing">
              <LandingSettingsCard
                ...
                savedSnapshot={savedLandingSnapshot}
